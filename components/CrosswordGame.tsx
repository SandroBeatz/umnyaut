  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm" onClick={onClose}>
    <MotionDiv initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center" onClick={e => e.stopPropagation()}>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={onClose} className={`${showLetters ? 'w-full' : 'flex-1'} py-4 bg-amber-500 text-white rounded-xl font-black shadow-lg`}>
          ПОНЯТНО
        </button>
      </div>
    </MotionDiv>
...
const CrosswordGame: React.FC<CrosswordGameProps> = ({ profile, crosswordData, onComplete, onCancel }) => {
  ...
  const [showHint, setShowHint] = useState<{ hint: string; word: string; showLetters: boolean } | null>(null);
  ...
  const isCellSolved = useCallback((r: number, c: number) => {
  ...
  const hasUnsolvedWord = useCallback((r: number, c: number) => {
    const words = getWordsForCell(r, c);
    return words.some(w => !solvedWordIds.has(w.idx.toString()));
  }, [solvedWordIds]);
  ...
  const handleCellClick = (r: number, c: number) => {
    ...
    if (focusedCell?.r === r && focusedCell?.c === c) {
      ...
      if (unsolvedWords.length > 1) {
        setActiveDirection(d => d === Direction.HORIZONTAL ? Direction.VERTICAL : Direction.HORIZONTAL);
      }
    } else {
      setFocusedCell({ r, c });
      if (unsolvedWords.length === 1) {
            ...
            {activeWord ? (
              <MotionDiv key={activeWord.idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
                <div className="flex justify-between items-start">
                  ...
                  <button onClick={() => {
                    setShowHint({ hint: activeWord.hint, word: activeWord.word, showLetters: false });
                    setHintsUsed(h => h + 1);
                  }}
