...
const Settings: React.FC<SettingsProps> = ({ profile, onSave }) => {
  ...
  return (
    <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
            className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold outline-none focus:border-indigo-500" />
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className="bg-violet-50 p-3 rounded-2xl"><ListFilter className="w-6 h-6 text-violet-600" /></div>
            <h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">Ваши интересы (макс. 5)</h3>
          </div>
          <button onClick={loadCategories} className="p-2 text-slate-400 hover:text-indigo-600 transition-colors"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /></button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {categories.map(cat => {
            ...
            return (
              <MotionButton key={cat.name} whileHover={{ scale: 1.05 }} onClick={() => toggleCategory(cat.name)}
      ...
      <div className="flex justify-center pt-4">
        <MotionButton onClick={handleSave} disabled={!username.trim() || selected.length === 0}
          className={`px-12 py-5 rounded-2xl font-black text-lg flex items-center gap-3 shadow-xl 
            ${username.trim() && selected.length > 0 ? 'bg-indigo-600 text-white border-b-4 border-indigo-800' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
