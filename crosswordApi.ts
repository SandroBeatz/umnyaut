 ...
 * Принимает опциональный объект guessedWords для расчёта прогресса.
 * Включает логику повторных попыток с экспоненциальной задержкой.
 */
export async function fetchCategories(
  guessedWords?: Record<string, string[]>,
  retries = 3
): Promise<Category[]> {
 ...
 * excludedWords - список уже отгаданных слов, которые не должны повторяться.
 */
